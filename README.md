...
- [x] Лимит активных записей (защита от злоупотреблений)
...
| `Error: BOT_TOKEN not set` | Файл `.env` не создан или не заполнен | Выполните шаг 8 |
...
/admin  (только для ADMIN_IDS)
  └── Админ-меню (inline-кнопки)
        ...
        │
        ├── 👤 Мастера
        │     └── Список мастеров + кнопка вкл/выкл (is_active toggle)
        │
        ├── ✂️ Услуги
        │     └── Список услуг + кнопка вкл/выкл
        │
        └── 🚫 Заблокировать время  [FSM]
              ├── Выбор мастера
              ├── Ввод периода текстом («2026-07-01 14:00 16:00 Обед»)
              ├── Подтверждение
              └── Список активных блокировок + кнопка «Удалить»
```
...
5. Для удаления блокировки: в меню блокировок нажмите «Удалить» рядом с нужной.
...
| `/admin` говорит «Нет доступа» | Ваш Telegram ID не в `ADMIN_IDS` | Добавьте свой ID в `.env` и перезапустите бота |
